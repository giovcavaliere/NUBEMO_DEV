  ...
  async function prepareAgendaForDashboardEvent(){
    ...
    agendaPreparing=(async()=>{
      ...
      if(!hasScript('professional-services.js'))
        await loadScript('professional-services.js?v=nubemo40clean01','Impossibile caricare i servizi Supabase dell’Agenda.');
      ...
      if(!hasScript('professional-settings-supabase-bridge.js'))
        await loadScript('professional-settings-supabase-bridge.js?v=nubemo40clean01','Impossibile preparare le impostazioni dell’Agenda.');
      await window.nubemoProfessionalSettingsBridge?.ready;

      if(!hasScript('professional-patient-lifecycle-bridge.js'))
        await loadScript('professional-patient-lifecycle-bridge.js?v=nubemo40clean01','Impossibile preparare i contatti dell’Agenda.');
      await window.nubemoPatientLifecycleBridge?.ready;

      if(!hasScript('professional-agenda-supabase-bridge.js')){
        await loadScript('professional-agenda-supabase-bridge.js?v=nubemo40clean01','Impossibile preparare l’Agenda.');
