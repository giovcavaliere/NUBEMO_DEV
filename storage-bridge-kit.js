// NUBEMO — kit condiviso per l'intercettazione di Storage.prototype.
//
...
//    recorder quelle scritture finivano sul vero localStorage e non
...
(() => {
  ...
  function patch(name, methods, claims) {
    ...
    for (const k of keys) {
      ...
      for (const value of pending) {
        try {
          proto.setItem.call(window.localStorage, k, value);
          replayed.push(k);
        } catch (error) {
          console.error('NUBEMO storage kit - replay fallito per', k, error);
        }
      }
    }
  ...
  window.NubemoStorageKit = Object.freeze({
