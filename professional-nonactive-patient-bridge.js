  ...
  function patchDetailsUi(){
    ...
    if(banner.dataset.pathwayStatus!==status){
      ...
      banner.innerHTML=status==='pending'
        ...
        : `<div class="section-head"><div><div class="eyebrow">STATO PERCORSO</div><h2>${statusLabel(status)}</h2></div><button class="secondary compact" type="button" data-new-ended-pathway>Nuovo percorso</button></div><p class="muted" style="margin:6px 0 0">La scheda resta consultabile in sola lettura. I percorsi precedenti sono disponibili nello Storico percorsi.</p>`;
    }
