// NUBEMO — bootstrap Area Paziente
// Supabase gestisce autenticazione e contesto.
(() => {
  'use strict';
  ...
  function loadPatientApp() {
    return loadScript('app.js?v=nubemo40clean01','Impossibile caricare l’Area Paziente.');
  }
  ...
  async function bootstrap() {
    try {
      ...
      await loadScript('patient-settings-supabase-bridge.js?v=nubemo40clean01','Impossibile applicare le impostazioni dell’Area Paziente.');
      await loadScript('patient-document-read-supabase-bridge.js?v=nubemo40clean01','Impossibile preparare lo stato di lettura dei documenti.');
      await window.nubemoPatientDocumentReadBridge?.ready;

      if (logoutButton) logoutButton.style.display = 'inline-flex';
      window.patientLogout = logout;

      await loadScript('food-catalog.js?v=nubemo40clean01','Impossibile caricare il catalogo alimenti.');
      await window.nubemoFoodCatalog.load(client);
      await loadPatientApp();
      await loadScript('patient-measures-pdf.js?v=nubemo40clean01','Impossibile preparare il PDF delle misurazioni.');
      window.nubemoPatientLegacyAdapter.bindLegacyApp?.();
      await loadScript('patient-recovery-contract.js?v=nubemo40clean01','Impossibile applicare il contratto dell’Area Paziente.');
