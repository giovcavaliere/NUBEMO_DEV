...
(() => {
  ...
  function context() { return window.nubemoProfessionalContext || {}; }
  function activePatients() { return Array.isArray(context().patients) ? context().patients : []; }
  function endedPatients() { return Array.isArray(context().endedPatients) ? context().endedPatients : []; }
  function allPatients() { return [...activePatients(), ...endedPatients()]; }
  function patientById(id) { return allPatients().find(row => row.id === id) || null; }
  ...
  function openEditDialog(patientId) {
    ...
    overlay.innerHTML = `
      <section class="clinical-modal">
        ...
        <div class="eyebrow">DATI PAZIENTE</div><h2>Modifica dati</h2>
        ...
        <label>Sesso</label><select id="pdSex">
        ...
        <div class="pro3-actions"><button class="secondary" id="cancelPatientDemographic" type="button">Annulla</button><button class="primary" id="savePatientDemographic" type="button">Salva modifiche</button></div>
      </section>`;
  ...
  function removeLegacyPatientActions() {
    ...
    const legacyMore = document.getElementById('patientMoreBtn');
    const legacyMenu = document.getElementById('patientMoreMenu');
    legacyMore?.remove(); legacyMenu?.remove();
  ...
  function ensurePatientMenu(patientId) {
    ...
    if (!row) return;
    const active = row.relationship?.status !== 'ended';
    ...
    wrap.innerHTML = `
      ...
      <div class="patient-more-menu" id="nubemoPatientActionsMenu" hidden>
        ${active ? '<button type="button" data-nubemo-patient-action="clinical">Modifica scheda</button>' : ''}
        ${active ? '<button type="button" data-nubemo-patient-action="demographics">Modifica anagrafica</button>' : ''}
        ${active ? '<button type="button" data-nubemo-patient-action="end">Termina percorso</button>' : '<span class="muted" style="display:block;padding:10px 12px">Percorso terminato</span>'}
      </div>`;
    ...
    menu?.querySelector('[data-nubemo-patient-action="clinical"]')?.addEventListener('click', () => {
      closePatientMenu(); window.nubemoProfessionalClinical?.openEditor?.(patientId);
    });
    menu?.querySelector('[data-nubemo-patient-action="demographics"]')?.addEventListener('click', () => {
      closePatientMenu(); openEditDialog(patientId);
    });
    menu?.querySelector('[data-nubemo-patient-action="end"]')?.addEventListener('click', () => {
      closePatientMenu(); void endPathway(patientId);
    });
  }
  ...
  function enhancePatientList() {
    ...
    card.className = 'card'; card.id = 'endedPatientsCard'; card.dataset.signature = signature;
    card.innerHTML = `<div class="section-head"><h2>Percorsi terminati</h2></div><div class="pro3-patients">
      ${rows.map(row => `<div class="pro3-patient" style="font-weight:400"><div class="patient-avatar">${esc(patientName(row).split(' ').map(x => x[0]).slice(0,2).join(''))}</div><div><span style="display:block;font-size:15px;font-weight:700;color:#34484f">${esc(patientName(row))}</span><span style="display:block;margin-top:3px;font-size:12px;color:#7b898f">Percorso terminato${row.relationship?.ended_at ? ` · ${new Date(row.relationship.ended_at).toLocaleDateString('it-IT')}` : ''}</span></div><button class="mini" type="button" data-reactivate-patient="${row.id}">Riattiva percorso</button></div>`).join('')}
      </div>`;
  ...
  function syncView() {
    ...
    patching = true;
    try {
      enhanceDetails(); enhancePatientList();
  ...
  function init() {
    ...
    const observer = new MutationObserver(() => queueMicrotask(syncView));
    observer.observe(app, { childList: true, subtree: true });
    syncView();
  }

  window.nubemoProfessionalPatients = Object.freeze({
    init, syncView, getActivePatients: activePatients, getEndedPatients: endedPatients,
    getCurrentPatientId: inferCurrentPatient, getPatientById: patientById,
    openEditDialog, endPathway
  });
  init();
})();
