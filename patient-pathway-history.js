...
(() => {
  ...
  function documentsContent(docs) {
    ...
    return `<div style="overflow-x:auto;-webkit-overflow-scrolling:touch"><div style="display:grid;gap:8px;min-width:520px">${rows.map(doc => `<div style="display:flex;align-items:center;justify-content:space-between;gap:14px;padding:10px 12px;border:1px solid #e4ebe8;border-radius:12px;background:#fbfdfc"><div style="min-width:0"><div style="font-weight:700;color:#2d4741;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(doc.title || doc.original_filename || 'Piano alimentare')}</div><div class="muted" style="margin-top:2px;font-size:12px">${fmtDate(doc.document_date || doc.created_at)}${doc.original_filename ? ` · ${esc(doc.original_filename)}` : ''}</div></div><button class="secondary compact" style="flex:0 0 auto;white-space:nowrap" type="button" data-open-patient-history-document="${esc(doc.id)}">Apri</button></div>`).join('')}</div></div>`;
  }
