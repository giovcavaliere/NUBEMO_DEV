// NUBEMO — Area Paziente: stato runtime non persistente.
// Supabase e' l'unica source of truth. Questo layer mantiene solo lo stato
// della sessione corrente e offre temporaneamente al motore UI legacy
// l'interfaccia sincrona che si aspetta, senza scrivere dati nel browser.
(() => {
  'use strict';
  ...
  function legacyProfile(ctx) {
    const p = ctx.profile || {}, patient = ctx.patient || {}, c = ctx.clinical || {}, settings = ctx.settings || {};
    return {
      firstName: p.first_name || '', name: p.first_name || '', surname: p.last_name || '',
      birth: patient.birth_date || '', height: patient.height_cm ?? '', sex: patient.sex || '',
      goal: c.goal_weight_kg ?? '', minWeight: c.min_weight_kg ?? '', maxWeight: c.max_weight_kg ?? '',
      reasonableWeight: c.reasonable_weight_kg ?? '', theoreticalWeight: c.theoretical_weight_kg ?? '',
      work: c.work || '', activity: c.activity || '', activityFactor: c.activity_factor ?? '',
      smoking: c.smoking || '', alcohol: c.alcohol || '', diagnosis: c.diagnosis || '', bowel: c.bowel || '',
      metabolism: c.metabolism || '', feeg: c.feeg || '', impedance: c.impedance || '',
      famObesity: !!c.family_obesity, famDiabetes: !!c.family_diabetes, famHypertension: !!c.family_hypertension,
      famCardiovascular: !!c.family_cardiovascular, famDyslipidemia: !!c.family_dyslipidemia, famThyroid: !!c.family_thyroid,
      previousDiets: c.previous_diets || '', allergies: c.allergies || '', medications: c.medications || '',
      giIssues: c.gi_issues || '', pastConditions: c.past_conditions || '', observations: c.observations || '', objectives: c.objectives || '',
      showEnergyValues: settings.showEnergyValues !== false,
      readOnly: !ctx.activePathway || settings.readOnly === true
    };
  }
  ...
  async function syncProfileSettings(serialized) {
    const profile = parse(serialized, {});
    if (!profile || typeof profile !== 'object') return;
    const current = context.settings && typeof context.settings === 'object' ? context.settings : {};
    const next = {
      ...current,
      showEnergyValues: profile.showEnergyValues !== false,
      readOnly: profile.readOnly === true
    };
    await services().saveSettings(context.patient.id, next);
  ...
  const storageFacade = Object.freeze({
    ...
    getItem(key) { const k=String(key); return memory.has(k) ? memory.get(k) : null; },
    setItem(key, value) {
      const k = String(key), v = String(value), previous = memory.get(k) ?? null;
      memory.set(k, v);
      if (k === KEY) {
      ...
      } else if (k === PROFILE_KEY) {
        settingsQueue = settingsQueue.then(() => syncProfileSettings(v)).catch(error => reportSyncError('impostazioni', error));
      }
    },
  ...
  async function openRemoteDocument(id) {
    ...
    try {
      ...
      await window.nubemoPatientDocumentReadBridge?.markRead?.(id);
  ...
  function bindRemoteDocumentsPage() {
    ...
    document.getElementById('savePatientDocument')?.addEventListener('click',async()=>{
      ...
      try{
        ...
        console.error('NUBEMO patient upload document:',error);
        alert('Non riesco a salvare il documento.');
        if(button){button.disabled=false;button.textContent='Salva documento';}
      }
    });
  ...
  function patchPrivacy() {
    ...
    card.querySelectorAll('[data-open-privacy]').forEach(button=>button.addEventListener('click',async()=>{
      ...
      try{
        const url=await services().openDocument(doc);
        openedPrivacyIds.add(doc.id);
        const accept=card.querySelector(`[data-accept-privacy="${doc.id}"]`);if(accept)accept.disabled=false;
        window.open(url,'_blank','noopener');
      }catch(error){console.error('NUBEMO privacy open:',error);alert('Non riesco ad aprire l’informativa.');}
    }));
    card.querySelectorAll('[data-accept-privacy]').forEach(button=>button.addEventListener('click',async()=>{
      ...
      try{
        await services().acceptPrivacy(context.profile.id,button.dataset.acceptPrivacy);
        privacyState=await services().loadPrivacy(context.profile.id);
        patchPrivacy();
      }catch(error){console.error('NUBEMO privacy accept:',error);alert('Non è stato possibile registrare l’accettazione.');button.disabled=false;}
    }));
  }
  ...
  async function flush() {
    await Promise.all([diaryQueue, measurementQueue, settingsQueue]);
  }

  window.nubemoPatientRuntimeStore = Object.freeze({
    init,
    flush,
    bindLegacyApp,
