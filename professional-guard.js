...
(() => {
  ...
  function perfEnd(timer) {
    const ms = Math.round(performance.now() - timer.startedAt);
    console.log(`[NUBEMO PERF] ${timer.label}: ${ms} ms`);
    return ms;
  }
  ...
  async function authorize() {
    ...
    try {
      ...
      timer = perfStart('Privacy gate');
      const privacyRequired = await requiresPrivacyGate(profile.id);
      perfEnd(timer);
      if (privacyRequired) return redirectToPrivacy();

      timer = perfStart('Profilo professionista');
      ...
      let logoData='';
      timer = perfStart('Logo professionista');
      ...
      timer = perfStart('Recovery contract');
      ...
      timer = perfStart('Patient settings');
      ...
      timer = perfStart('Professional settings');
      ...
      timer = perfStart('Note professionista');
      ...
      timer = perfStart('Stato lettura documenti');
      ...
      timer = perfStart('Piani alimentari');
      ...
      timer = perfStart('Catalogo alimenti');
      ...
      timer = perfStart('Privacy professionista');
