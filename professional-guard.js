...
(() => {
  ...
  async function ensureFullRuntime(){
    ...
    fullRuntimePromise=(async()=>{
      ...
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');
      ...
      timer=perfStart('Diario calorie persistite lazy');if(!hasScript('professional-diary-calorie-supabase-bridge.js'))await loadScript('professional-diary-calorie-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le calorie persistite del Diario.');perfEnd(timer);
      timer=perfStart('Adapter 3A lazy');if(!hasScript('professional-legacy-supabase-adapter.js'))await loadScript('professional-legacy-supabase-adapter.js?v=nubemo40clean02','Impossibile preparare i dati dell’Area Professionista.');if(!window.nubemoProfessionalLegacyAdapter)throw new Error('Adattatore dati PRO non inizializzato.');await window.nubemoProfessionalLegacyAdapter.init(window.nubemoProfessionalContext);window.nubemoProfessionalDiaryCaloriesBridge?.installStorageOverlay?.();perfEnd(timer);
      timer=perfStart('Lifecycle lazy');if(!hasScript('professional-patient-lifecycle-bridge.js'))await loadScript('professional-patient-lifecycle-bridge.js?v=nubemo40clean02','Impossibile preparare i contatti e l’accesso paziente.');await window.nubemoPatientLifecycleBridge?.ready;perfEnd(timer);
      timer=perfStart('Recovery contract lazy');if(!hasScript('professional-recovery-contract.js'))await loadScript('professional-recovery-contract.js?v=nubemo40clean02','Impossibile applicare il contratto runtime dell’Area Professionista.');perfEnd(timer);
      timer=perfStart('Patient settings lazy');if(!hasScript('professional-patient-settings-supabase-bridge.js'))await loadScript('professional-patient-settings-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le impostazioni dell’Area Paziente.');await window.nubemoProfessionalPatientSettingsBridge?.ready;perfEnd(timer);
      timer=perfStart('Professional settings lazy');if(!hasScript('professional-settings-supabase-bridge.js'))await loadScript('professional-settings-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le impostazioni del professionista.');await window.nubemoProfessionalSettingsBridge?.ready;perfEnd(timer);
      timer=perfStart('Note bridge lazy');if(!hasScript('professional-notes-supabase-bridge.js'))await loadScript('professional-notes-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le note del professionista.');perfEnd(timer);
      timer=perfStart('Documenti metadata lazy');if(!hasScript('professional-documents-supabase-bridge.js'))await loadScript('professional-documents-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare i documenti del paziente.');await window.nubemoProfessionalDocumentsBridge?.ready;perfEnd(timer);
      timer=perfStart('Stato lettura documenti lazy');if(!hasScript('professional-document-read-supabase-bridge.js'))await loadScript('professional-document-read-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare lo stato di lettura dei documenti.');await window.nubemoProfessionalDocumentReadBridge?.ready;perfEnd(timer);
      timer=perfStart('Piani bridge lazy');if(!hasScript('professional-plans-supabase-bridge.js'))await loadScript('professional-plans-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare i piani alimentari.');perfEnd(timer);
      timer=perfStart('Esami bridge lazy');if(!hasScript('professional-labs-supabase-bridge.js'))await loadScript('professional-labs-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare gli esami del paziente.');perfEnd(timer);
      timer=perfStart('Gestione pazienti lazy');if(!hasScript('professional-patient-management.js'))await loadScript('professional-patient-management.js?v=nubemo40clean02','Impossibile caricare la gestione dei pazienti.');perfEnd(timer);
      timer=perfStart('Account e Privacy paziente lazy');if(!hasScript('professional-access-privacy-supabase-bridge.js'))await loadScript('professional-access-privacy-supabase-bridge.js?v=nubemo40clean02','Impossibile caricare Account e Privacy del paziente.');perfEnd(timer);
      timer=perfStart('Privacy professionista lazy');if(!hasScript('professional-profile-privacy-supabase-bridge.js'))await loadScript('professional-profile-privacy-supabase-bridge.js?v=nubemo40clean02','Impossibile caricare il PDF privacy del professionista.');perfEnd(timer);
  ...
  async function ensurePatientSummaryLoader(){
    ...
    patientSummaryLoaderPromise=(async()=>{
      if(!window.nubemoProfessionalPatientSummaryLazy)await loadScript('professional-patient-summary-lazy.js?v=nubemo40clean02','Impossibile preparare il riepilogo paziente.');
  ...
  async function ensureLabsTab(patientId){
    ...
    labsTabPromise=(async()=>{const timer=perfStart('Esami tab lazy');try{
      ...
      if(!hasScript('professional-documents-supabase-bridge.js'))await loadScript('professional-documents-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare i referti del paziente.');await window.nubemoProfessionalDocumentsBridge?.ready;await window.nubemoProfessionalDocumentsBridge?.ensurePatient?.(patientId);
      if(!hasScript('professional-document-read-supabase-bridge.js'))await loadScript('professional-document-read-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare lo stato di lettura dei referti.');await window.nubemoProfessionalDocumentReadBridge?.ready;
      if(!hasScript('professional-labs-supabase-bridge.js'))await loadScript('professional-labs-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare gli esami del paziente.');await window.nubemoProfessionalLabsBridge?.ready;await window.nubemoProfessionalLabsBridge?.ensurePatient?.(patientId);
  ...
  async function ensurePlanTab(patientId){
    ...
    planTabPromise=(async()=>{const timer=perfStart('Piano tab lazy');try{
      ensureSelectedPatientContext(patientId);
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      if(!hasScript('professional-documents-supabase-bridge.js'))await loadScript('professional-documents-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare i documenti del piano.');await window.nubemoProfessionalDocumentsBridge?.ready;await window.nubemoProfessionalDocumentsBridge?.ensurePatient?.(patientId);
      if(!hasScript('professional-plans-supabase-bridge.js'))await loadScript('professional-plans-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare i piani alimentari.');await window.nubemoProfessionalPlansBridge?.ready;window.nubemoProfessionalPlansBridge?.setCurrentPatient?.(patientId);await window.nubemoProfessionalPlansBridge?.ensurePatient?.(patientId);
  ...
  async function ensureDocumentsTab(patientId){
    ...
    documentsTabPromise=(async()=>{const timer=perfStart('Documenti tab lazy');try{
      ensureSelectedPatientContext(patientId);
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      if(!hasScript('professional-documents-supabase-bridge.js'))await loadScript('professional-documents-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare i documenti del paziente.');await window.nubemoProfessionalDocumentsBridge?.ready;await window.nubemoProfessionalDocumentsBridge?.ensurePatient?.(patientId);
      if(!hasScript('professional-document-read-supabase-bridge.js'))await loadScript('professional-document-read-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare lo stato di lettura dei documenti.');await window.nubemoProfessionalDocumentReadBridge?.ready;
  ...
  async function ensureAccountTab(patientId){
    ...
    accountTabPromise=(async()=>{const timer=perfStart('Account tab lazy');try{
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      await ensurePatientAccessContext(patientId);
      if(!hasScript('professional-access-privacy-supabase-bridge.js'))await loadScript('professional-access-privacy-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare l’account del paziente.');if(!window.nubemoProfessionalAccessPrivacyBridge)throw new Error('Bridge Account non inizializzato.');window.nubemoProfessionalAccessPrivacyBridge.setCurrentPatient?.(patientId);
  ...
  async function ensureDiaryTab(patientId){
    ...
    diaryTabPromise=(async()=>{const timer=perfStart('Diario tab lazy · 30 giorni');try{
      ...
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');
      if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      if(!hasScript('professional-diary-calorie-supabase-bridge.js'))await loadScript('professional-diary-calorie-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le calorie persistite del Diario.');
      if(!hasScript('professional-patient-diary-lazy.js'))await loadScript('professional-patient-diary-lazy.js?v=nubemo40clean02','Impossibile preparare il Diario del paziente.');
  ...
  async function ensureTrendTab(patientId){
    ...
    trendTabPromise=(async()=>{const timer=perfStart('Andamento tab lazy · storico pesi');try{
      ensureSelectedPatientContext(patientId);
      if(!hasScript('professional-patient-trend-lazy.js'))await loadScript('professional-patient-trend-lazy.js?v=nubemo40clean02','Impossibile preparare l’andamento del paziente.');
  ...
  async function ensureMeasuresTab(patientId){
    ...
    measuresTabPromise=(async()=>{const timer=perfStart('Misure tab lazy');try{
      ...
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');
      if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      if(!hasScript('professional-measures-supabase-bridge.js'))await loadScript('professional-measures-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le misure del paziente.');
  ...
  async function ensureVisitsTab(patientId){
    ...
    visitsTabPromise=(async()=>{const timer=perfStart('Visite tab lazy');try{
      ...
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');
      if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      if(!hasScript('professional-visits-supabase-bridge.js'))await loadScript('professional-visits-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le visite del paziente.');
  ...
  async function ensureNotesTab(patientId){
    ...
    notesTabPromise=(async()=>{const timer=perfStart('Note tab lazy');try{
      ...
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');
      if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      if(!hasScript('professional-notes-supabase-bridge.js'))await loadScript('professional-notes-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le note del paziente.');
  ...
  async function ensureAgenda(){
    ...
    agendaPromise=(async()=>{const timer=perfStart('Agenda lazy');try{
      ...
      if(!hasScript('professional-services.js'))await loadScript('professional-services.js?v=nubemo40clean02','Impossibile caricare i servizi Supabase dell’Area Professionista.');
      if(!window.nubemoProfessionalServices)throw new Error('Servizi Supabase Area Professionista non inizializzati.');
      if(!hasScript('professional-settings-supabase-bridge.js'))await loadScript('professional-settings-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le impostazioni dell’Agenda.');
      await window.nubemoProfessionalSettingsBridge?.ready;
      if(!hasScript('professional-patient-lifecycle-bridge.js'))await loadScript('professional-patient-lifecycle-bridge.js?v=nubemo40clean02','Impossibile preparare i contatti dell’Agenda.');
      await window.nubemoPatientLifecycleBridge?.ready;
      if(!hasScript('professional-agenda-supabase-bridge.js'))await loadScript('professional-agenda-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare l’Agenda.');
  ...
  async function ensureProfessionalProfile(){
    ...
    professionalProfilePromise=(async()=>{const timer=perfStart('Profilo professionista lazy');try{
      ...
      if(!hasScript('professional-settings-supabase-bridge.js'))await loadScript('professional-settings-supabase-bridge.js?v=nubemo40clean02','Impossibile preparare le impostazioni del professionista.');
      await window.nubemoProfessionalSettingsBridge?.ready;
  ...
  async function authorize(){
    ...
    try{
      ...
      timer=perfStart('Dashboard bridge');await loadScript('professional-dashboard-bootstrap.js?v=nubemo40clean02','Impossibile preparare la Dashboard.');perfEnd(timer);
      ...
      timer=perfStart('pro.js');await loadScript('pro.js?v=nubemo40clean02','Impossibile caricare l’Area Professionista.');perfEnd(timer);
