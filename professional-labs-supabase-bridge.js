...
(() => {
  ...
  function install(){if(installed)return;installed=true;storageProto.getItem=function(key){const k=String(key);if(this===window.localStorage&&(k===LABS_KEY||k===PENDING_LABS_KEY))return memory.has(k)?memory.get(k):null;return previousGetItem.call(this,key);};storageProto.setItem=function(key,value){const k=String(key);if(this!==window.localStorage||(k!==LABS_KEY&&k!==PENDING_LABS_KEY))return previousSetItem.call(this,key,value);memory.set(k,String(value));if(k===LABS_KEY)queue=queue.then(()=>syncLabs(String(value))).catch(error=>{console.error('NUBEMO PRO Supabase sync (esami):',error);window.dispatchEvent(new CustomEvent('nubemo:supabase-sync-error',{detail:{domain:'esami',message:error?.message||String(error)}}));});};storageProto.removeItem=function(key){const k=String(key);if(this===window.localStorage&&(k===LABS_KEY||k===PENDING_LABS_KEY)){memory.delete(k);return;}return previousRemoveItem.call(this,key);};}
